...
**注意**：虽然测例很简单，但提醒读者 spawn **不必** 像 fork 一样复制父进程的地址空间。
...
- 通过所有测例。
  ...
  max{runtimes/prio}/ min{runtimes/prio} < 1.5.
...
**注意**：注意，由于本章内核默认将 `ch5b_initproc` 作为初始测例运行，所以必须用 `BASE=2` 而非之前章节的 `BASE=0` 加载。

**提示**：利用 `git cherry-pick` 系列指令，能方便地将前一章分支 commit 移植到本章分支。
...
- 完成问答题。
- 加入 honorcode 的内容。否则，你的提交将视作无效，本次实验的成绩将按"0"分计。
- 推荐markdown文档格式。
